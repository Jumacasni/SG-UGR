...
class MyScene extends THREE.Scene {
  constructor (unRenderer) {
    ...
    this.maximoGiroPendulo = Math.PI/2;
    this.minimoGiroPendulo = -Math.PI/2;

    this.pendulo1SentidoHorario = true;
    this.pendulo2SentidoHorario = true;
  }
  ...
  createGUI () {
    ...
    folderanimacion.add (this.guiControls, 'velocidad1', 0, 5, 1).name('Pendulo 1: ');
    folderanimacion.add (this.guiControls, 'velocidad2', 0, 5, 1).name('Pendulo 2: ');
  }
  ...
  update () {
    ...
    if(this.pendulo1SentidoHorario){
    	this.primerpendulo.rotation.z += this.guiControls.velocidad1 * segundosTranscurridos;
    	
    	if(this.primerpendulo.rotation.z > this.maximoGiroPendulo)
    		this.pendulo1SentidoHorario = false;
    }

    else{
    	this.primerpendulo.rotation.z -= this.guiControls.velocidad1 * segundosTranscurridos;

    	if(this.primerpendulo.rotation.z < this.minimoGiroPendulo)
    		this.pendulo1SentidoHorario = true;
    }
    ...
    if(this.pendulo2SentidoHorario){
    	this.segundopendulo.rotation.z += this.guiControls.velocidad2 * segundosTranscurridos;
    	
    	if(this.segundopendulo.rotation.z > this.maximoGiroPendulo)
    		this.pendulo2SentidoHorario = false;
    }

    else{
    	this.segundopendulo.rotation.z -= this.guiControls.velocidad2 * segundosTranscurridos;

    	if(this.segundopendulo.rotation.z < this.minimoGiroPendulo)
    		this.pendulo2SentidoHorario = true;
    }
